...
st.markdown("<div class='main-title'>🧠 TumorVision AI</div>", unsafe_allow_html=True)
st.markdown("<div class='sub-title'>Clinical-Grade Brain MRI Diagnostic Assistant</div>", unsafe_allow_html=True)

@st.cache_resource
def load_medical_model():
    model_path = "new_brain_tumor_DenseNet121.keras" 
...
with col_uploader:
    uploaded_file = st.file_uploader(
        "Securely upload a patient's MRI scan (JPG/PNG)", 
        type=["jpg", "jpeg", "png"]
    )
...
if uploaded_file is not None:
    ...
    with analysis_col:
        with st.container(border=True):
            ...
            if analyze_button:
                progress_text = "Scanning brain textures..."
                ...
                if winning_class != "No Tumor":
                    st.error("⚠️ **Immediate neurologist review recommended.**")
                else:
...
with st.expander("⚖️ Medical Disclaimer & Model Information", expanded=False):
    st.warning(
        ...
        "professional medical advice, diagnosis, or treatment. Always consult a board-certified neurologist."
    )
    st.info("**Model Architecture:** Transfer Learning Baseline v1.0 | **Training Accuracy:** ~87%")

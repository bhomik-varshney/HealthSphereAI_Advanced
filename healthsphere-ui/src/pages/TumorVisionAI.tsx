...
import { predictTumorDisease } from '@/services/api';
...
export default function TumorVisionAI() {
  ...
  const fullText = "Clinical-grade brain MRI diagnostic assistant powered by advanced deep learning technology";
  ...
  const handleRunDiagnostics = async () => {
    ...
    try {
      const predictionResult = await predictTumorDisease(selectedFile);
      setResult(predictionResult);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to process MRI image');
    } finally {
      setLoading(false);
    }
  };
  ...
  const getDiagnosisIcon = (diagnosis: string) => {
    const isNormal = diagnosis.toLowerCase() === 'no tumor';
    return isNormal ? (
            ...
            <Card className="h-full border-0 shadow-lg bg-white rounded-2xl overflow-hidden">
              <CardHeader className="border-b-2 border-teal-200 bg-gradient-to-r from-teal-100 via-emerald-100 to-cyan-100 py-5">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <CardTitle className="text-xl font-bold text-teal-700">Upload MRI Image</CardTitle>
              ...
              {!previewUrl ? (
                <label className="flex items-center justify-center w-full h-64 px-4 transition bg-slate-50 border-2 border-dashed rounded-2xl appearance-none cursor-pointer border-primary/30 hover:bg-primary/5 hover:border-primary group">
                  <div className="flex flex-col items-center justify-center pt-5 pb-6">
                    ...
                    <p className="text-sm font-semibold text-foreground mb-1">Click to select MRI</p>
            ...
            {loading ? (
              <Card className="flex items-center justify-center min-h-96 border-0 shadow-lg bg-gradient-to-br from-teal-50/50 to-emerald-50/50 rounded-2xl overflow-hidden">
                <CardContent className="text-center py-12">
                  ...
                  <p className="text-muted-foreground text-center max-w-xs text-lg font-medium">
                    Analyzing your MRI image with advanced AI diagnostics...
                  </p>
                </CardContent>
              </Card>
            ) : !result ? (
              <Card className="h-full flex items-center justify-center min-h-96 border-0 shadow-lg bg-gradient-to-br from-gray-50 to-slate-50 rounded-2xl overflow-hidden">
                <CardContent className="text-center py-12">
                  ...
                  <p className="text-muted-foreground text-center max-w-xs text-lg font-medium">
                    Upload an MRI image and run diagnostics to see results
                  </p>
                </CardContent>
              </Card>
            ) : (
                ...
                <Card className="border-0 shadow-lg bg-white rounded-2xl overflow-hidden">
                  ...
                  <CardContent className="space-y-6 pt-6">
                    ...
                    <div className="rounded-lg bg-secondary/50 p-4 overflow-hidden">
                      <p className="text-sm text-muted-foreground mb-3">Tumor Type Probability Breakdown</p>
                      <div className="space-y-3">
                        {Object.entries(result.probabilities).map(([tumor, probability]) => (
                          <div key={tumor} className="w-full">
                            <div className="flex justify-between items-center mb-1 gap-2">
                              <span className="text-sm text-foreground truncate">{tumor}</span>
          ...
          <CardContent className="pt-8">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
              ...
              <div className="rounded-2xl bg-gradient-to-br from-blue-50 to-cyan-50 p-6 border-2 border-blue-200">
                ...
                <p className="text-sm text-muted-foreground">Select and upload a brain MRI image in JPG or PNG format</p>
              </div>
              ...
              <div className="rounded-2xl bg-gradient-to-br from-purple-50 to-pink-50 p-6 border-2 border-purple-200">
                ...
                <p className="text-sm text-muted-foreground">Advanced deep learning model analyzes the MRI scan in real-time</p>
              </div>
          ...
          <CardHeader className="border-b-2 border-teal-200 bg-gradient-to-r from-teal-100 via-emerald-100 to-cyan-100 py-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <CardDescription className="font-semibold text-teal-600">Brain tumors identified by TumorVision AI</CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent className="pt-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div className="rounded-2xl bg-gradient-to-br from-red-50 to-orange-50 p-6 border-2 border-red-200">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <h4 className="font-bold text-red-700">Glioma</h4>
                </div>
                <p className="text-sm text-muted-foreground">Detects glioma tumors including high-grade glioblastomas and lower-grade gliomas with precise localization</p>
              </div>
              ...
              <div className="rounded-2xl bg-gradient-to-br from-orange-50 to-amber-50 p-6 border-2 border-orange-200">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <h4 className="font-bold text-orange-700">Meningioma</h4>
                </div>
                <p className="text-sm text-muted-foreground">Identifies meningioma tumors arising from the protective membranes surrounding the brain</p>
              </div>
              ...
              <div className="rounded-2xl bg-gradient-to-br from-yellow-50 to-amber-50 p-6 border-2 border-yellow-200">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <h4 className="font-bold text-yellow-700">Pituitary Tumor</h4>
                </div>
                <p className="text-sm text-muted-foreground">Detects pituitary gland tumors affecting hormone regulation and endocrine function</p>
              </div>
              ...
              <div className="rounded-2xl bg-gradient-to-br from-green-50 to-emerald-50 p-6 border-2 border-green-200">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <h4 className="font-bold text-green-700">No Tumor</h4>
                </div>
                <p className="text-sm text-muted-foreground">Confirms normal brain MRI with no tumor detected or neoplastic abnormalities</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <div className="flex gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                This AI diagnostic tool is intended to assist healthcare professionals and should not be used as a substitute for professional medical advice, diagnosis, or treatment. Always consult with a qualified neurologist or physician for accurate interpretation of medical imaging and clinical decisions. The results provided by this tool are predictions and should be verified by licensed medical professionals.
              </p>
            </div>
          </div>
        </div>
